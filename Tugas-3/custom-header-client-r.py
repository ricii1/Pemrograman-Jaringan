...
'''
Setting Custom Headers (http.client)
Description: Write a Python program that makes a GET request to
https://httpbin.org/headers with a custom header X-Test-Header:
TestValue. Parse the response and print the value of X-Test-Header
that was received by the server. Use http.client module to connect
to the server.
Input: None.
Output (without unit test): The value of X-Test-Header sent to the
server.
Output (with unit test):
connection called with: call('httpbin.org')
request called with: call('GET', '/headers', headers={'X-TestHeader': 'TestValue'})
read called: b'{"headers": {"X-Test-Header": "TestValue"}}'
connection closed: call()
test attribute passed: TestValue is equal to TestValue'''
